...
select * , row_number() over(partition by city order by cost desc) as 'dense_rank' from restaurants;
...
select * ,concat( row_number() over(partition by city order by cost desc) , ' - ' , name) as 'rank' from restaurants;


-- 7. Find top 5 restaurants of every city as per their revenue
    select * from (      
          select *, 
            rating_count*cost as 'revenue' , row_number() over (partition by city order by rating_count*cost desc ) as 'rank'
             from restaurants
      ) t1 where t1.rank <=5;       
...
select * from (      
          select *, 
            rating_count*cost as 'revenue' , row_number() over (partition by cuisine order by rating_count*cost desc ) as 'rank'
             from restaurants
      ) t1 where t1.rank <=5;       
...
with
	q_1 as ( select sum(cost*rating_count) as 'top_revenue'
			from (select *, cost*rating_count ,row_number() over(order by cost*rating_count desc) as 'rank' from restaurants) t
			where t.rank <= (((select count(*) from restaurants)/100)*20)),
            
	q_2 as ( select sum(rating_count*cost) as 'total_revenue' from restaurants)
SELECT round((top_revenue/total_revenue)*100) as 'revenue %' FROM q_1, q_2;
