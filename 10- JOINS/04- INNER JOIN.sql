...
-- inner join | Userdetails & Transaction
select * from userdetails inner join transaction on userdetails.user_id = transaction.user_id;

-- inner join | Transaction & Orders
select * from transaction inner join orders on transaction.transaction_id = orders.transaction_id;

-- inner join | Userdetails , Transaction & Orders
select * from userdetails inner join transaction on userdetails.user_id = transaction.user_id 
inner join orders on transaction.transaction_id = orders.transaction_id;


-- inner join | select specific columns | Userdetails , Transaction & Orders
select userdetails.user_id , userdetails.user_name , userdetails.email, transaction.transaction_id , 
transaction.transaction_amount, orders.order_id , orders.order_date from userdetails 
inner join transaction on userdetails.user_id = transaction.user_id 
inner join orders on transaction.transaction_id = orders.transaction_id;
