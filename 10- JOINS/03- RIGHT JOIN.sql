...
-- right join , Userdetails & Transaction 
select * from userdetails right join transaction on userdetails.user_id = transaction.user_id;

-- right join , Transaction & Orders
select * from transaction right join orders on transaction.transaction_id = orders.transaction_id;

-- right join , Userdetails , Transaction & Orders
select * from userdetails right join transaction on userdetails.user_id = transaction.user_id 
right join orders on transaction.transaction_id = orders.transaction_id;

-- right join | select specific columns| Userdetails , Transaction & Orders
select userdetails.user_id , userdetails.user_name , userdetails.email, transaction.transaction_id , 
transaction.transaction_amount, orders.order_id , orders.order_date 
from userdetails right join transaction on userdetails.user_id = transaction.user_id 
right join orders on transaction.transaction_id = orders.transaction_id;
